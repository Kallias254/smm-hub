...
export const FullScreenChat: React.FC<FullScreenChatBlock & { primaryColor: string, tenantName?: string, tenantLogo?: any }> = ({ 
  ...
  const prefilledVariables = useMemo(() => ({
    ...
    'agencyName': tenantName || 'Our Agency',
    'Agency Name': tenantName || 'Our Agency',
    'agency_name': tenantName || 'Our Agency',
    'PrimaryColor': primaryColor,
    'primaryColor': primaryColor,
  }), [tenantName, primaryColor])
        ...
        <Center style={{ position: 'absolute', inset: 0, zIndex: 10000, backgroundColor: 'inherit' }}>
          <Stack align="center" gap="md">
             {tenantLogo?.url ? (
                ...
                <Text fw={700} size="xl" style={{ letterSpacing: '2px', opacity: 0.5 }}>{tenantName?.toUpperCase() || 'SMM HUB'}</Text>
             )}
